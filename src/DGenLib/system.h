...
static __inline uint16_t h2le16(uint16_t v)
{
...
static __inline uint16_t h2be16(uint16_t v)
{
#ifdef WORDS_BIGENDIAN
	return v;
#else
...
static __inline uint32_t h2le32(uint32_t v)
{
#ifdef WORDS_BIGENDIAN
...
static __inline uint32_t h2be32(uint32_t v)
{
#ifdef WORDS_BIGENDIAN
	return v;
#else
...
static __inline uint24_t *u24cpy(uint24_t *dst, const uint24_t *src)
{
...
static __inline size_t strcommon(const char *s1, const char *s2)
{
	size_t i;
